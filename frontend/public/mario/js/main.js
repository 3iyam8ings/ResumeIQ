...
async function main(canvas) {
    ...
    async function setupLevel(name) {
        ...
        // level.comp.layers.push(createCollisionLayer(level)); // DEBUG: uncomment to show collision boxes
    ...
    timer.update = function update(deltaTime) {
        ...
        if (!gameContext.gameOverEmitted && mario) {
            const killable = mario.traits.get(Killable);
            const playerTimer = mario.traits.get(LevelTimer);
            const playerState = mario.traits.get(Player);

            if ((killable && killable.dead) || (playerTimer && playerTimer.currentTime <= 0) || mario.pos.y > 300) {
                gameContext.gameOverEmitted = true;
                timer.stop();
                window.parent.postMessage({ type: 'GAME_OVER', coins: playerState ? playerState.coins : 0 }, '*');
            }

            if (playerState && gameContext.tick % 10 === 0) {
                window.parent.postMessage({ type: 'COIN_UPDATE', coins: playerState.coins }, '*');
            }
        }
    }
